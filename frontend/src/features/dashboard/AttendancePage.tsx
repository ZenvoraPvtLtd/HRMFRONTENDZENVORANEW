...
import { fetchHrAttendance, HrAttendanceRow } from "../../services/attendanceApi";
...
export default function Attendance({ employeePortal = false }: AttendanceProps) {
  ...
  useEffect(() => {
    ...
    fetchHrAttendance({ start: date })
      .then((res) => {
        // Map HrAttendanceRow to AttendanceRecord
        const mapped = res.data.map(r => ({
          ...
          marked: r.clock_in ? new Date(r.clock_in).toLocaleTimeString() : "-",
  ...
  }, [attendanceRecords, department, employee, search, status]);
