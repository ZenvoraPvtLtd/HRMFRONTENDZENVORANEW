...
const TERMINATION_REASONS = {
  multiple_faces: "Interview terminated because more than one person was detected.",
  no_face: "Interview terminated because candidate left camera multiple times.",
  background_voice: "Interview terminated due to repeated background voice violations.",
  tab_switch: "Interview terminated because tab switching was detected.",
  fullscreen_exit: "Interview terminated because fullscreen mode was exited repeatedly.",
...
export default function InterviewRoom() {
  ...
  const cheatTimerTimeoutRef = useRef<number | null>(null);
  ...
  // Stable ref so the face-monitor interval always calls the latest version of the termination function.
  const multiFaceTerminateRef = useRef<() => void>(() => {});
  ...
  const beginInterview = async () => {
    ...
    console.log(`[SESSION] interview_id=${id} → candidate joined`);
    console.log(`[SESSION] interview_id=${id} → participant_count=2 (candidate + AI)`);
  ...
  const terminateForProctoring = (
    ...
    console.log(`[SESSION] interview_id=${id} → terminate (proctoring: ${warningType})`);

    if (countViolation) {
      violationCountsRef.current[warningType] += 1;
      setViolationBreakdown((prev) => ({ ...prev, [warningType]: violationCountsRef.current[warningType] }));
    ...
    browserProctorStatusRef.current = "Terminated";
    setCameraProctorStatus("Terminated");
    setAudioProctorStatus("Terminated");
    setBrowserProctorStatus("Terminated");
  ...
  const terminateForTabSwitch = () => {
    terminateForProctoring(
      "tab_switch",
      "tab_switch_terminated",
      TERMINATION_REASONS.tab_switch,
      violationCountsRef.current.tab_switch === 0
    );
  };

  const terminateForMultipleFaces = () => {
    console.log("[FACE] terminateForMultipleFaces called");
    console.log("[FACE] terminatedRef.current at entry:", terminatedRef.current);
    console.log("[FACE] candidate:", candidate, "| id:", id);
    if (terminatedRef.current) {
      console.log("[FACE] already terminated — skipping");
      return;
    }
    // Immediate termination — bypass debounce, bypass violation count check.
    terminatedRef.current = true;
    const reason = TERMINATION_REASONS.multiple_faces;
    console.log(`[SESSION] interview_id=${id} → terminate (proctoring: multiple_faces) IMMEDIATE`);
    ...
    // Save backend proctoring event (no debounce guard for multi-face).
    if (candidate && id) {
      interviewApi.saveProctoringEvent({
        candidate_id: candidate.id,
        interview_id: Number(id),
        event_type: "multiple_faces_terminated",
        severity: "critical",
        message: reason,
      }).catch((err) => console.error("Failed to save multiple_faces_terminated event:", err));
    }

    if (id) {
      interviewApi.terminate(Number(id)).catch((err) =>
        console.error("Failed to mark interview terminated (multiple faces):", err)
      );
    }

    loggedProctoringRef.current.multiple_faces = true;
    loggedProctoringRef.current.multiple_faces_terminated = true;
  };
  // Keep the ref in sync so the face-monitor interval always calls the freshest closure.
  multiFaceTerminateRef.current = terminateForMultipleFaces;
  ...
  const recordProctoringViolation = (
    ...
    } else if (eventType === "tab_switch" || eventType === "fullscreen_exit" || eventType === "paste_detected") {
      browserProctorStatusRef.current = "Warning";
      setBrowserProctorStatus("Warning");
    } else {
  ...
  const startCheatTimerIfNeeded = (type: "face" | "multi-face" | "noise" | "tab" | "fullscreen" | "paste") => {
    if (terminatedRef.current) return;
    ...
    if (cheatStartMsRef.current == null) {
      ...
      setCheatType(type);
      if (cheatTimerTimeoutRef.current != null) {
        window.clearTimeout(cheatTimerTimeoutRef.current);
      }
      cheatTimerTimeoutRef.current = window.setTimeout(() => {
        if (terminatedRef.current || !cheatViolationActiveRef.current || pausedByCheatRef.current) return;
        triggerCheatPause(
          type === "face"
            ...
            : type === "multi-face"
              ...
              : type === "noise"
                ...
                : type === "fullscreen"
                  ? "Fullscreen was exited. Please return to fullscreen mode."
                  : type === "paste"
                    ? "Pasting answers is not allowed."
  ...
  const clearCheatTimer = () => {
    ...
    if (cheatTimerTimeoutRef.current != null) {
      window.clearTimeout(cheatTimerTimeoutRef.current);
      cheatTimerTimeoutRef.current = null;
    }
    setCheatType(null);
  ...
  useEffect(() => {
    ...
    const hasActiveBrowserViolation = () =>
      loggedProctoringRef.current.tab_switch ||
      loggedProctoringRef.current.fullscreen_exit ||
      loggedProctoringRef.current.paste_detected;

    const clearBrowserWarningIfResolved = () => {
      if (hasActiveBrowserViolation()) return;
      if (!pausedByCheat) {
        setCheatWarning("");
        browserProctorStatusRef.current = "OK";
        setBrowserProctorStatus("OK");
      }
    };

    const clearBrowserTimerIfResolved = () => {
      if (!hasActiveBrowserViolation()) {
        clearCheatTimer();
      }
    };

    const recordBrowserViolation = (eventType: Extract<WarningEventType, "tab_switch" | "fullscreen_exit" | "paste_detected">) => {
      if (eventType === "tab_switch") {
        terminateForTabSwitch();
        return;
      }

      const message = PROCTORING_MESSAGES[eventType];
      setCheatWarning(message);
      setShowCheatingPopup(true);
      browserProctorStatusRef.current = "Warning";
      setBrowserProctorStatus("Warning");
      recordProctoringViolation(eventType, message, "warning");
    };
    ...
    const onVis = () => {
      ...
      if (document.visibilityState !== "visible") {
        ...
        recordBrowserViolation("tab_switch");
        startCheatTimerIfNeeded("tab");
      } else {
        tabCheatTriggeredRef.current = false;
        clearProctoringEvent("tab_switch");
        clearBrowserWarningIfResolved();
        clearBrowserTimerIfResolved();
      }
    };
    ...
    const onFullscreenChange = () => {
      if (!interviewStartedRef.current || !fullscreenExitArmedRef.current) return;
      if (!document.fullscreenElement) {
        recordBrowserViolation("fullscreen_exit");
        startCheatTimerIfNeeded("fullscreen");
      } else {
        clearProctoringEvent("fullscreen_exit");
        clearBrowserWarningIfResolved();
        clearBrowserTimerIfResolved();
      }
    };
    ...
    const onPaste = (event: ClipboardEvent) => {
      ...
      clearProctoringEvent("paste_detected");
      recordBrowserViolation("paste_detected");
      window.setTimeout(() => {
        clearProctoringEvent("paste_detected");
        clearBrowserWarningIfResolved();
      }, 1500);
  ...
  useEffect(() => {
    ...
    const monitor = createFaceMonitor({
      video,
      config: { persistSeconds: 0, intervalMs: 250 },
      onWarning: (status) => {
        console.log("[FACE] onWarning status:", status);
        console.log("[FACE] interviewStartedRef.current:", interviewStartedRef.current);
        console.log("[FACE] terminatedRef.current:", terminatedRef.current);

        // Do not act on face events before the interview has actually started.
        if (!interviewStartedRef.current) return;
        if (terminatedRef.current) return;

        if (status === "multiple-faces") {
          console.log("[FACE] multiple-faces detected — calling terminateForMultipleFaces immediately");
          setDetectionStatus("multiple-people");
          // Call via ref so we always get the freshest closure (avoids stale candidate/id).
          multiFaceTerminateRef.current();
    ...
    (async () => {
      try {
        ...
        console.error("[FACE] Face detector failed to initialize:", err);
        setFaceDetectorFailed(true);
        console.warn("[FACE] Monitor will run without real detection — only simulator mode works.");
        // Always start the monitor even on init failure so simulator still works
        faceMonitorRunningRef.current = true;
        monitor.start?.();
      }
    })();
  ...
  useEffect(() => {
    ...
    // Terminate / cleanup when the candidate closes/refreshes the tab mid-session.
    const handleBeforeUnload = () => {
      if (id && !terminatedRef.current) {
        // Fire-and-forget via sendBeacon so it works even on tab close.
        const url = `/api/ai-interview/interview/${id}/terminate`;
        navigator.sendBeacon?.(url);
        console.log(`[SESSION] interview_id=${id} → terminate (beforeunload)`);
      }
    };
    ...
    return () => {
      ...
      // Mark session terminated when component unmounts without normal completion.
      if (id && !terminatedRef.current) {
        interviewApi.terminate(Number(id)).catch(() => {});
        console.log(`[SESSION] interview_id=${id} → terminate (unmount)`);
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const triggerCheatPause = (reason: string, type: "face" | "multi-face" | "noise" | "tab" | "fullscreen" | "paste") => {
    if (terminatedRef.current) return;
    if (pausedByCheat) return;
    setCheatType(type);
    setCheatWarning(reason);
  ...
  const submitAndNext = async () => {
    ...
    try {
      ...
      if (idx + 1 < questions.length) {
        ...
        // All questions answered — mark complete, then analyze.
        terminatedRef.current = true; // prevent double-terminate on unmount
        if (id) {
          await interviewApi.complete(Number(id)).catch(() => {});
          console.log(`[SESSION] interview_id=${id} → completed (all questions answered)`);
        }
        stopInterviewMedia();
        const result = await interviewApi.analyze(candidate.id, Number(id));
  ...
  const handleTimerExpire = useCallback(() => {
    if (terminatedRef.current) return;
    console.log(`[SESSION] interview_id=${id} → terminate (timer expired)`);
    terminatedRef.current = true;
    setTerminated(true);
    setTerminationReason("Interview time limit reached. The session has been automatically terminated.");
    setPhase("terminated");
    stopInterviewMedia();
    if (id) {
      interviewApi.terminate(Number(id)).catch((err) =>
        console.error("Failed to mark interview terminated on timer expiry:", err)
      );
    }
    // Auto-analyze whatever was answered.
    if (candidate && id) {
      interviewApi.analyze(candidate.id, Number(id))
        .then((result) => nav(`/candidate/interview/${id}/result`, { state: { result, terminated: true } }))
        .catch(() => nav(`/candidate/interview/${id}/result`, { state: { terminated: true } }));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [candidate, id, nav]);
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Timer seconds={(questions.length || 5) * 120} onExpire={handleTimerExpire} />
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className={`font-semibold ${proctoringStatusClass(browserProctorStatus)}`}>
                    {browserProctorStatus}
                  </span>
                </div>
                <div className="flex justify-between items-center text-sm">
                ...
                <div className="grid grid-cols-3 gap-2 text-[11px]">
                  ...
                  <div className="rounded border border-slate-100 bg-slate-50 p-2 text-center">
                    <div className="text-slate-500">Tab switch</div>
                    <div className="font-bold text-slate-800">{violationBreakdown.tab_switch} / 2</div>
                  </div>
                  <div className="rounded border border-slate-100 bg-slate-50 p-2 text-center">
                    <div className="text-slate-500">Fullscreen</div>
                    <div className="font-bold text-slate-800">{violationBreakdown.fullscreen_exit} / 2</div>
                  </div>
                  <div className="rounded border border-slate-100 bg-slate-50 p-2 text-center">
                    <div className="text-slate-500">Paste</div>
                    <div className="font-bold text-slate-800">{violationBreakdown.paste_detected} / 2</div>
                  </div>
                </div>
